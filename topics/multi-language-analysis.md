...
<link-summary>This manual provides instructions and guidance on using OpenGrep within Qodana, 
including troubleshooting and configuration tips based on known behaviors.</link-summary>

OpenGrep is a powerful, fast, and reliable static analysis engine integrated into Qodana. This feature searches for vulnerabilities and security issues such as SQL Injection, XSS, Command Injection, Path Traversal, and similar problems.
It supports multiple languages and is designed for high-performance analysis, including taint analysis (DFA).

## Key Features

### OpenGrep Support
OpenGrep supports analysis for various languages, including Java, .NET, JavaScript, TypeScript, and others. 
    It performs deep analysis using Taint Analysis (DFA) to detect security vulnerabilities and code quality issues.

### Configurability
You can control OpenGrep behavior by configuring which rules are applied:
*   **Disabling Built-in Rules**: If OpenGrep is consuming excessive resources or reporting irrelevant issues,
    you can disable default rules using configuration options.

## Performance and Troubleshooting

### Performance Tuning
While OpenGrep is optimized for speed, analysis of very large codebases or complex rules (like PQC rules) might take longer.
...
When using OpenGrep in .NET or Docker environments:
*   Ensure OpenGrep is initialized correctly. If analysis hangs or fails, verify the environment setup.
...
*   **Rule Clashes**: Ensure that any forked versions of OpenGrep do not clash with upstream installations on your system (`~/.cache/opengrep`, `~/.opengrep`).
...
OpenGrep integrates Security analysis capabilities powered by OpenGrep inspections, addressing the need for automated security vulnerability checks in modern, AI-generated code.
