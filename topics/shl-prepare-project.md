...
To add and map users, ensure that a user role or group is included in the OIDC access token received during authentication.
Create role mapping using the following command:

```Bash
curl --location 'https://<qodana_deployment_url>/v1/organizations/<organization-id>/sso-roles/' \
--header 'Cookie: user-cookie-session=<token value>' \
--header 'Content-Type: application/json' \
--data '{
    "ssoRole": "<okta-group-value>",
    "role": "<role-value>",
    "additionalRoles": []
}'
```
In this snippet, replace the following placeholders:

| Placeholder               | Replace with                                                                         |
|---------------------------|--------------------------------------------------------------------------------------|
| `<qodana_deployment_url>` | Backend URL of your %product% deployment. For example,  `api.qodana.local`           |
| `<organization-id>`       | %product% organization ID                                                            |
| `<token value>`           | Valid user session token. To get it, discover browser cookies logged in to %product% |
| `<oidc-group-name>`       | Group name from your OIDC provider                                                   |
| `<qodana-role>`           | %cloud% role, see [](cloud-user-roles.md) for details                                |

For example, after all replacements made this command may look like:

```Bash
curl --location 'https://api.qodana.local/v1/organizations/p577D/sso-roles/' \
--header 'Cookie: user-cookie-session=token%3D%dummy' \
--header 'Content-Type: application/json' \
--data '{
    "ssoRole": "developers",
    "role": "EDITOR",
