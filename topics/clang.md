...
<var name="compdb" value="`compile_commands.json`">
...
<link-summary>%qp% lets you analyze C and C++ projects that provide a %compdb% file.</link-summary>
...
%qp% lets you analyze C and C++ projects that provide a [%compdb% file](https://clang.llvm.org/docs/JSONCompilationDatabase.html). There are two different linters which provide this functionality:

- `qodana-clang`, which is available under the Community license, and supports only [Clang-Tidy](https://clang.llvm.org/extra/clang-tidy)-based inspections;
- `qodana-cpp`, which is available under the Ultimate and Ultimate Plus licenses, and supports the full set of inspections provided by CLion (e.g. [MISRA](https://en.wikipedia.org/wiki/MISRA_C) inspections).
...
The Docker image of `qodana-clang` employs Clang 16. You can see the [`Dockerfile`](%dockerfile%) for the detailed description of all software used by the linter.

The linter searches for compile commands in the `build/compile_commands.json` file of the project directory. This file is usually generated by your build system (see below). After reading the `compile_commands.json` file, %product% analyzes the project, generates analysis reports and saves them locally or uploads to Qodana Cloud.
...
<procedure>
    ...
    <step>
        <p>
          For `qodana-clang`, you need to generate <code>compile_commands.json</code> as explained in the <a href="%compdb-generate%">CLion documentation portal</a>, and save it to the <code>build</code> directory under the project root.</p>
        <p>
...
As mentioned in [prerequisites](#Prepare+your+project), `qodana-clang` reads the `build/compile_commands.json` file and runs the Clang-Tidy tool, and `qodana-cpp` picks up any build system that is supported by CLion and runs analysis on the default build target.
...
<tabs group="software">
    <tab title="GitHub Actions" group-key="github">
      ...
      to your repository, save this workflow configuration to the <code>.github/workflows/code_quality.yml</code> file:</p>
        ...
        <p>To override the location of <code>compile_commands.json</code>, you can specify the location relative to the project root, so the configuration would look like:
        </p>
          <code-block lang="yaml">
              name: Qodana
              on:
...
<note>`qodana-clang` does not support <a href="qodana-yaml.md" anchor="Include+an+inspection+into+the+analysis+scope">including</a> and <a href="qodana-yaml.md" anchor="exclude-paths">excluding</a> paths for specific inspections through the <code>qodana.yaml</code> file.</note>

### Modifying paths for analysis

<note>
  This section is only applicable to `qodana-clang` and to rare cases where `qodana-cpp` is analyzing a project configured via a raw `compile_commands.json` file.
</note>
