...
This page provides the list of changes in %product% as of version 2022.3.

## New linters

Starting from 2022.3, %product% has the [Qodana for .NET](qodana-dotnet.md) and [Qodana for Go](qodana-go.md) linters.

## Qodana for JVM

This is the list of new inspections that were added to the [Qodana for JVM](qodana-jvm.md) linter.
...
|`VoidMethodAnnotatedWithGET`| Warning | REST: `@GET` annotated method returns void value |
|`WadlDomInspection`| Error | Incorrect WADL configuration|

In 2022.3, the following inspections:

* `SpringJavaConstructorAutowiringInspection` 
* `SpringJavaAutowiredMembersInspection`  

were merged into `SpringJavaInjectionPointsAutowiringInspection`.
...
|`SpringShellReferencesInspection`| Warning | Invalid Spring Shell references Non-bundled |
...
|`ValidExternallyBoundObject`| Error | Valid externally bound object com.jetbrains.jax.ws unbundled |
...
Here is the list of new inspections of the [Qodana Community for JVM](qodana-jvm-community.md) linter.
...
|`ConstantValue`| (disabled by default) Warns about conditions that are always true, including those in the `assert` statements |
...
|`IncorrectPluginDslStructure`| Gradle: Detects disallowed statements before `plugins {}` block |
...
Several inspections were removed from the linter:
...
|`InspectionUniqueToolbarId`| Warning  |Specify toolbar id|
...
Several inspections were added to the [Qodana Community for Android](qodana-jvm-android.md) in 2022.3.
...
|`AndroidLintPermissionNamingConvention`| Warning  |Permission name does not follow recommended convention|
...
A couple of inspections were removed from this linter.
...
| Inspection name                                            | Description                                                  |
|------------------------------------------------------------|--------------------------------------------------------------|
| `BehatDocStepCanBeConvertedToAttributeInspection`          | Behat doc step can be converted to attribute version for creating shorter tests |
...
| `PhpRedeclarationStdlibFunctionInspection`                 | Redeclaration of standard library function. It is a runtime error to redeclare a function from the PHP standard library, such as `is_array` |
...
| `PhpRegExpInvalidDelimiterInspection`                      | Invalid delimiter. For example, in `preg_match('treats', 'sweets');`, the `t` is the delimiter of the regular expression, the regular expression is equivalent to `'/rea/s` |
| `PhpTaintFunctionInspection`                               | Vulnerabilities scanner                                      |
...
Several inspections were added to [Qodana for JS](qodana-js.md) as per 2022.3.
...
A couple of inspections were renamed as shown in this table.
...
In 2022.3, several inspections from the unbundled CoffeeScript, Haml, and W3C Validators plugins were deleted. 
...
| `Query_index_required` | Reports GQL queries for which an index is not defined in `index.yaml`. Such queries will fail on the production server |
| `Query_bound_parameters` | Reports GQL queries with bound parameters that don't have the necessary parameters passed to the query method call |
...
Twenty-five new CSS-related inspections were added to the [Qodana for Go](qodana-go.md), 
[Qodana for .NET](qodana-dotnet.md), and [Qodana for Python](qodana-python.md) linters. 
...
Here is the list of SQL inspections that were added to 2022.3 in all linters except the Qodana Community for JVM linter.
...
By default, these inspections are disabled.
