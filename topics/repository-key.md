...
<link-summary>A repository key is a public part of an SSH key pair generated by %cloud% during the 
onboarding stage that provides %cloud% with secure read-only access to your repository and lets %product% count 
contributors, which is required by our license agreement. </link-summary>

A repository key is a public part of an SSH key pair generated by %cloud% during the 
[project setup](Quick-start.topic#quickstart-prerequisites) stage. When you create a new [project](cloud-projects.topic) in %cloud%,
the organization wizard requires an SSH or HTTPS URL to your repository. After you provide such URL, %cloud% generates 
