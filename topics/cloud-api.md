...
* You already have a %cloud% organization, see the [](cloud-organizations.topic#cloud-organizations-create-organization) chapter for details.
...
* In API call examples, replace the `{qodana.cloud.url}` placeholder with your base URL, i.e. with `qodana.cloud` for %cloud%, or using your custom base URL in case of %premlite%.
...
To create a new team along with a project and obtain its [project token](project-token.md), send a `POST` request using the 
`https://{qodana.cloud.url}/api/v1/public/organizations/teams/projects` endpoint, for example: 
...
```cURL
curl -X GET \
   "https://{qodana.cloud.url}/api/v1/public/organizations/users?limit=10" \
   -H "Authorization: Bearer $PERMANENT_ORGANIZATION_TOKEN" \
   -H "Content-Type: application/json"
```
