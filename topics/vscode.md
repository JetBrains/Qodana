...
<link-summary>Using Visual Studio Code, you can run %product%, explore %product% reports and connect to %cloud%. </link-summary>
...
* [Connect to %cloud%](#vs-code-connect-to-qodana-cloud)
* [Explore %product% reports](#vs-code-explore-reports) local or downloaded from %cloud%
* [Configure %product% in Visual Studio Code](#vs-code-configure-extension)

## Before you start

<link-summary>Make sure that you have a project in %cloud% and Java 11 or later is installed on your machine. </link-summary>
...
Download and install the **%product%** extension from the 
...
Alternatively, in the command palette type `%product%` to see the list of options present in the **%product%** view.
...
<procedure>
    <step>
        ...
        <img src="vs-code-local-run.png" alt="Clicking the Run Locally button in the %product% view" width="304" border-effect="line"/>
    </step>
    <step>
        <p>In the dialog that opens, enter the <a href="project-token.md">project token</a> that was generated by %cloud%.</p>
...
To forward %product% reports to %cloud% and get the latest %product% reports, log in to %cloud% and connect 
your project opened in the IDE to a specific %cloud% [project](cloud-projects.topic) as shown below.
...
<procedure>
   <step>
      ...
      <img src="vs-code-cloud-login-1.png" alt="Logging in to %cloud%" width="304" border-effect="line"/>
   </step>
   <step>
      <p>
         In the <ui-path>%product%</ui-path> view, select a project in %cloud% that you would like to link  
...
<table>
    ...
    <tr>
        ...
        <p>Full Path = Workspace + Path Prefix + Path in a <a href="qodana-inspection-output.md" anchor="SARIF+Output">SARIF</a> file</p>
...
<p>In a <a href="ui-overview.md">%cloud% report</a>, you can check with the <ui-path>Files</ui-path> section to 
...
<a href="ui-overview.md" anchor="ui-overview-actual-problems">%cloud% report</a> using the 
<ui-path>Open file in ...</ui-path> button and choosing Visual Studio Code as the tool for opening. </p>
<p>Alternatively, the Project ID value is contained in your %cloud% project URL. This URL has the following structure: 
...
* **Attached to report** means that the report was downloaded from %cloud%, and now you can
  [view](#vs-code-explore-reports) it using Visual Studio Code.

## Troubleshooting
{id="vs-code-troubleshooting"}

<link-summary>Use "Qodana: Reset authentication" to switch between %cloud% accounts, and use "Qodana: Reset all settings"
